...
class RecurrentDeformationNet(nn.Module):
    def __init__(self, in_channels, out_channels, model_path=None):
        super().__init__()
        self.downsample = nn.Sequential(
                        nn.Conv3d(in_channels, 32, kernel_size=(1, 5, 5), stride=(1, 2, 2), 
        ...
        self.upsample = nn.Sequential(
            ...
            nn.LeakyReLU(0.2, inplace=True),
            deconv(32, out_channels, 3, 1)
        )

        # edge loss
        self.edgeDetector = EdgeDetection(in_ch=out_channels, out_ch=1, mid_ch=16)
    ...
    def forward(self, depth_maps, flows=None):
        b, t, _, h, w = depth_maps.size()
        depth_maps = depth_maps.permute(0,2,1,3,4)
        
        if flows is not None:
            flows = flows.permute(0,2,1,3,4)
            inputs = torch.cat((depth_maps, flows), dim=1)
        else:
            inputs = torch.cat((depth_maps), dim=1)
        ...
        predicted_depth_maps = self.upsample(feat_d1)
        if self.training:
            predicted_edges = self.edgeDetector(predicted_depth_maps)
            predicted_edges = predicted_edges.view(b, t, 1, h, w)
        else:
            predicted_edges = None

        predicted_depth_maps = predicted_depth_maps.view(b, t, 3, h, w)

        return predicted_depth_maps, predicted_edges
