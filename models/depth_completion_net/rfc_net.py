...
class RecurrentDepthCompleteNet(nn.Module):
    ...
    def forward(self, masked_flows, masks, shapes=None):
        # masked_flows: b t-1 2 h w
        # masks: b t-1 2 h w
        masked_flows = masked_flows * (1 - masks)
        b, t, _, h, w = masked_flows.size()
        masked_flows = masked_flows.permute(0,2,1,3,4)
        masks = masks.permute(0,2,1,3,4)
        
        if shapes is not None:
            shapes = shapes.permute(0,2,1,3,4)
            inputs = torch.cat((masked_flows, masks, shapes), dim=1)
        else:
