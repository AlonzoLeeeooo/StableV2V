...
import torch
import random
from models.anydoor.cldm.model import create_model, load_state_dict
from models.anydoor.cldm.ddim_hacked import DDIMSampler
from models.anydoor.datasets.data_utils import get_bbox_from_mask, pad_to_square, expand_image_mask, sobel, expand_bbox, box2squre, box_in_box
cv2.setNumThreads(0)
...
def inference_single_image(ref_image, ref_mask, tar_image, tar_mask, guidance_scale, config_path, anydoor_checkpoint_path, seed):
    config = OmegaConf.load(config_path)
    model_config = config.config_file

    model = create_model(model_config).cpu()
    model.load_state_dict(load_state_dict(anydoor_checkpoint_path, location='cpu'))
...
def anydoor_inference_runner(args):
    os.makedirs(args.outdir, exist_ok=True)
    ref_image = cv2.imread(args.reference_image)
    ref_image = cv2.cvtColor(ref_image, cv2.COLOR_BGR2RGB)
    ref_image = cv2.resize(ref_image, (args.width, args.height))

    gt_image = cv2.imread(os.path.join(args.source_video_frames, sorted(os.listdir(args.source_video_frames))[0]))
    gt_image = cv2.cvtColor(gt_image, cv2.COLOR_BGR2RGB)
    gt_image = cv2.resize(gt_image, (args.width, args.height))
    ...
    tar_mask = Image.open(os.path.join(args.input_masks, sorted(os.listdir(args.input_masks))[0])).convert('P')
    tar_mask = tar_mask.resize((args.width, args.height))
    tar_mask = np.array(tar_mask)


    gen_image = inference_single_image(ref_image,
                                       ref_mask,
                                       gt_image.copy(),
                                       tar_mask,
                                       guidance_scale=args.guidance_scale,
                                       config_path=args.anydoor_config_path,
                                       anydoor_checkpoint_path=args.anydoor_checkpoint_path,
                                       seed=args.seed)

    save_path = os.path.join(args.outdir, 'image_editing_results')
    os.makedirs(save_path, exist_ok=True)
    filename = args.prompt.lower().replace('.', '').replace(' ', '_') + '.png'
    cv2.imwrite(os.path.join(save_path, filename), gen_image[:, :, ::-1])
